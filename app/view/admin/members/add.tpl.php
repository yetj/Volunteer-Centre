...
<form class="needs-validation" action="<?= adminURL('members', 'add_post') ?>" method="post" novalidate>
  ...
  <div class="form-row">
    ...
    <div class="col-md-4 mb-3">
      ...
      <div class="input-group">
        ...
        <input type="text" class="form-control" placeholder="Nazwa" value="<?= isset($post['name']) ? $post['name'] : '' ?>" name="name" aria-describedby="inputGroupPrepend" required>
      </div>
    </div>
  </div>
  <div class="form-row">
  ...
  <div class="form-row">
  	...
  	<table class="table table-sm table-bordered">
  		...
  		<tbody>
  			<tr class="text-center">
  				<td><input type="checkbox" name="access[]" id="PROJECTS" value="<?= MemberModel::ADMIN_ACCESS_PROJECTS ?>" <?= isset($post['access']) ? in_array(MemberModel::ADMIN_ACCESS_PROJECTS, $post['access']) ? 'checked="checked"' : '' : '' ?>></td>
  				<td><input type="checkbox" name="access[]" id="MEMEBERS" value="<?= MemberModel::ADMIN_ACCESS_MEMEBERS ?>" <?= isset($post['access']) ? in_array(MemberModel::ADMIN_ACCESS_MEMEBERS, $post['access']) ? 'checked="checked"' : '' : '' ?>></td>
  				<td><input type="checkbox" name="access[]" id="TEMPLATES" value="<?= MemberModel::ADMIN_ACCESS_TEMPLATES ?>" <?= isset($post['access']) ? in_array(MemberModel::ADMIN_ACCESS_TEMPLATES, $post['access']) ? 'checked="checked"' : '' : '' ?>></td>
  				<td><input type="checkbox" name="access[]" id="LOGS" value="<?= MemberModel::ADMIN_ACCESS_LOGS ?>" <?= isset($post['access']) ? in_array(MemberModel::ADMIN_ACCESS_LOGS, $post['access']) ? 'checked="checked"' : '' : '' ?>></td>
  				<td><input type="checkbox" name="access[]" id="ALL_PROJECTS" value="<?= MemberModel::ADMIN_ACCESS_ALL_PROJECTS ?>" <?= isset($post['access']) ? in_array(MemberModel::ADMIN_ACCESS_ALL_PROJECTS, $post['access']) ? 'checked="checked"' : '' : '' ?>></td>
  			</tr>
  		</tbody>
  	</table>
  	</div>
		...
		<h5>Projekty</h5>
			<? if($projects): ?>
				<? foreach($projects as $project): ?>
        		  <div class="input-group">
        			<label><input type="checkbox" name="projects[]" value="<?= $project->id ?>" <?= isset($post['projects']) ? in_array($project->id, $post['projects']) ? 'checked="checked"' : '' : '' ?>/> <strong><?= $project->slug ?></strong> - <?= $project->name ?></label>
        		  </div>
		  		<? endforeach ?>
		  	<? endif ?>
	  </div>
