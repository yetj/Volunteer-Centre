<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
	<h1 class="h2">Projekt - Edycja</h1>
</div>
<h5><a href="<?= adminURL('projects') ?>">&laquo; wróć do listy projektów</a></h5>

<form class="needs-validation" action="<?= adminURL('projects', 'edit_post', ['id' => $project->id]) ?>" method="post" novalidate>
  ...
  <div class="form-row">
    ...
    <div class="col-md-2 mb-3">
      ...
      <select name="status" class="form-control" <?= $project->slug == "global" ? 'disabled' : '' ?>>
  ...
  <div class="form-row">
    ...
    <div class="col-md-4 mb-3">
      <label for="validationCustom03">Właściciel</label>
      	<select name="owner" class="form-control" id="validationCustom03" <?= $project->slug == "global" ? 'disabled' : '' ?>>
      		<option value="0">-- brak właściciela --</option>
      		<? foreach($members as $member): ?>
      			<option value="<?= $member->id ?>"><?= $member->access > 0 ? '*' : ''; ?> <?= $member->login ?> (<?= $member->email ?>)</option>
      		<? endforeach ?>
      	</select>
      <div class="invalid-feedback">
